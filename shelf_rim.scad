...
rim_channel_width = shelf_z+4; //channel width of the rim based on the shelf thickness it will slide on
...
rim_height_bottom = 4; //height of the rim which is the depth of the rail
rim_height_top = rim_height_bottom*2;
rim_base_height = rim_height_bottom;


pn_depth = 10; // depth of the part number cut (arbitrary)
pversion = "shelf_rim 0.1a perek";
...
difference() { 
    union() {
        //Chamfer_3d(dr=chamfer_radius){
            ...
            color("purple")
                translate([0,-1*rim_width/2+rim_bottom_rail_width/2,rim_base_height])
                    cube([shelf_length, rim_bottom_rail_width, rim_height_bottom], center = true);

            color("purple")
                translate([0,-1*rim_width/2+rim_bottom_rail_width+rim_channel_width,rim_base_height])
                    cube([shelf_length, rim_bottom_rail_width, rim_height_top], center = true);
    }
    
    color("green")
        translate([0,rim_width/4,0]) mirror([0,0,0])        
